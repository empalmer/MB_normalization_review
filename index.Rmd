...
Due to the sequencing technology, samples will have different sequencing depths, or the sum of all the counts in a sample. Directly comparing raw counts between samples is not possible. To illustrate this, consider the counts of one taxon, labeled `taxon1`, across two samples shown below. In Sample A, this taxon has a count of 230, and in Sample B, this taxon has a count of 23. Is this taxon differentially abundant between samples? As we see below, the way we normalize the data can change how we would answer this question. 


```{r, demo_data_table_raw_counts, echo = F}
sampleA <- c(230,5)
sampleB <- c(23,5)
ASVs <- c("taxon1", "taxon2")
tibble(ASVs, sampleA, sampleB) %>% 
    knitr::kable(col.names = c("Sample A", "Sample B", "Taxa"), 
...
p1 <- data.frame(sampleA, sampleB, ASVs) %>% 
    ...
    ggtitle("Observed raw counts of 2 taxa in 2 samples", 
            subtitle = "The raw counts of taxon 1 are different, and the raw counts of taxon 2 are equal. Sampling depth is not accounted for") + 
    theme_minimal() + 
...
p2 <- data.frame(sampleA = sampleA/sum(sampleA), sampleB = sampleB/sum(sampleB), ASVs) %>% 
    ...
    ggplot(aes(y = Proportion, x = ASVs, fill = sample)) + 
        geom_bar(stat = "identity", position = position_dodge()) + 
    ggtitle("Normalized counts of 2 taxa in 2 samples", 
            subtitle = "When library size is accounted for, the difference between sample A and B appears smaller, and there now appears to be a difference in taxon 2, even though the raw counts are identical")+ 
    theme_minimal() + 
    theme(legend.position="bottom", 
          legend.title = element_blank(), 
          axis.title.x = element_blank()) + 
